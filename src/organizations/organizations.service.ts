...
export class OrganizationsService {
  constructor(private prismaService: PrismaService) {} //must write this at the beginning
  async create(createOrganizationDto: CreateOrganizationDto) {
    ...
    if (await this.checkIfOrganizationExist(createOrganizationDto.name)) {
      throw new BadRequestException(
        `Organization ${createOrganizationDto.name}`,
      );
    }
  ...
  async update(id: number, updateOrganizationDto: UpdateOrganizationDto) {
    ...
    if (
      ...
      throw new BadRequestException(
        `Organization ${updateOrganizationDto.name} has already been taken`,
      );
    }
