...
export class ItemsService {
  ...
  async update(id: number,organization_id: number, updateItemDto: UpdateItemDto) {
    const itemOrganization =await this.getItemById(id, organization_id)
    ...
    if(!await this.checkIfItemExist(updateItemDto.name,id)){
      throw new BadRequestException(`Item ${updateItemDto.name} has already been taken`)
    }

    return this.prismaService.item.update({
      where: {id: itemOrganization.item_id,},
      data:updateItemDto,
      }
      );
  }
  ...
  private async getItemById(id: number, organization_id: number) {
    const item = await this.prismaService.itemOrganization.findFirst({
      where: { item_id: id, organization_id: organization_id },
      include: {
        item: true,
      },
    });
